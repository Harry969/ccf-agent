...
## Replace Chapter Skill

Set `chapter_skill.path` to a chapter-specific skill when you want a different method or algorithm structure. The default skill is tuned for top-conference and CCF-style algorithm chapters: it expects a pipeline, local definitions before equations, pseudocode with bounded symbols, and a complexity paragraph.
