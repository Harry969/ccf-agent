...
def render_prompt(config: AgentConfig) -> str:
    ...
    chapter_skill = config.get("chapter_skill", {})
    ...
    chapter_skill_path = chapter_skill.get("path")
    chapter_skill_text = config.read_text(chapter_skill_path) if chapter_skill_path else ""
    if chapter_skill_text:
        sections.append("## Algorithm Chapter Skill\n" + chapter_skill_text.strip())
